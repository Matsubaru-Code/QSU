...
local updater = {
    current_version = 1,              -- Числовая версия вашего скрипта
    version_text = "1.00",            -- Текстовое отображение версии
    update_json_url = "https://raw.githubusercontent.com/Matsubaru-Code/QSU/refs/heads/main/update.json",
    script_url = "https://raw.githubusercontent.com/ВАШ_НИК/ВАШ_РЕПО/main/qsu.lua",
...
function updater.checkForUpdate()
    lua_thread.create(function()
        local response = requests.get(updater.update_json_url)
        if response.status_code ~= 200 then
            sampAddChatMessage("{FF0000}[QSU]: Не удалось проверить обновления (код " .. response.status_code .. ").", -1)
            return
        end

        local ok, data = pcall(json.decode, response.text)
        if not ok or not data then
            sampAddChatMessage("{FF0000}[QSU]: Ошибка парсинга JSON обновлений.", -1)
            return
        end

        local remote_version = tonumber(data.version) or 0
        if remote_version > updater.current_version then
            updater.update_available = true
            local news = data.news or "Обновление доступно."
            sampAddChatMessage(string.format(
                "{FFA500}[QSU]: Доступна новая версия %s (текущая %s). Обновление...",
                tostring(data.version_text or remote_version),
                updater.version_text
            ), -1)
            sampAddChatMessage("{FFA500}[QSU]: " .. news, -1)
            
            -- Скачиваем и устанавливаем новый скрипт
            updater.downloadAndInstall()
        else
            sampAddChatMessage("{00FF00}[QSU]: Скрипт актуален (версия " .. updater.version_text .. ").", -1)
        end
    end)
end
...
function updater.downloadAndInstall()
    ...
    downloadUrlToFile(updater.script_url, script_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            sampAddChatMessage("{00FF00}[QSU]: Скрипт обновлён! Перезагрузка...", -1)
            thisScript():reload()
        else
            sampAddChatMessage("{FF0000}[QSU]: Ошибка загрузки нового скрипта (статус: " .. tostring(status) .. ").", -1)
        end
    end)
end
